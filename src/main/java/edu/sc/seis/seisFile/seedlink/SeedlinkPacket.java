...
public class SeedlinkPacket {
    ...
    public SeedlinkPacket(byte[] bytes) {
        if (bytes.length != HEADER_SIZE + DATA_SIZE) {
            throw new IllegalArgumentException("Nunber of bytes must be "+(HEADER_SIZE + DATA_SIZE)+" but given "+bytes.length);
        }
        ...
        if ( ! header.startsWith(SL_PREFIX)) {
            throw new IllegalArgumentException("SeedLink packets must start with 'SL', not '"+header.substring(0, SL_PREFIX.length()));
        }
